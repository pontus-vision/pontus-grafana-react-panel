...
var PVGrid = /** @class */ (function (_super) {
    ...
    function PVGrid(props) {
        ...
        _this.componentDidMount = function () {
            ...
            _this.on("" + _this.namespace + (_this.subNamespace ? _this.subNamespace : '') + "-pvgrid-on-search-changed", _this.setSearch);
            _this.on("" + _this.namespace + (_this.subNamespace ? _this.subNamespace : '') + "-pvgrid-on-search-exact-changed", _this.setSearchExact);
            _this.on("" + _this.namespace + (_this.subNamespace ? _this.subNamespace : '') + "-pvgrid-on-col-settings-changed", _this.setColumnSettingsCb);
            _this.on("" + _this.namespace + (_this.subNamespace ? _this.subNamespace : '') + "-pvgrid-on-extra-search-changed", _this.setExtraSearch);
        };
        _this.getSearchObj = function (from, to, searchstr, searchExact, cols, dataType, sortcol, sortdir, filters, customFilter) {
            return {
                search: {
                    searchStr: searchstr,
        ...
        _this.ensureData = function (fromReq, toReq) {
            ...
            _this.hRequest = setTimeout(function () {
                ...
                Axios.post(url, self.getSearchObj(fromReqNum, toReq, self.searchstr, self.searchExact, self.cols, self.dataType, self.sortcol, self.sortdir, self.filters, self.customFilter), {
                    headers: {
        ...
        _this.onSuccessPVRestQuery = function (resp) {
            var e_1, _a;
            var from = resp.data.from, to = from + resp.data.records.length;
            var items = [];
        ...
        _this.setDataType = function (str) {
            _this.dataType = str;
            PontusComponent.setItem("" + _this.namespace + (_this.subNamespace ? _this.subNamespace : '') + ".PVGrid.dataType", _this.dataType);
        };
        ...
        _this.setColumnSettings = function (colSettings) {
            ...
            if (colSettings) {
                PontusComponent.setItem("" + _this.namespace + (_this.subNamespace ? _this.subNamespace : '') + ".PVGrid.colSettings", JSON.stringify(colSettings));
        ...
        _this.componentWillUnmount = function () {
            ...
            _this.off("" + _this.namespace + (_this.subNamespace ? _this.subNamespace : '') + "-pvgrid-on-search-changed", _this.setSearch);
            _this.off("" + _this.namespace + (_this.subNamespace ? _this.subNamespace : '') + "-pvgrid-on-search-exact-changed", _this.setSearchExact);
            _this.off("" + _this.namespace + (_this.subNamespace ? _this.subNamespace : '') + "-pvgrid-on-col-settings-changed", _this.setColumnSettingsCb);
            _this.off("" + _this.namespace + (_this.subNamespace ? _this.subNamespace : '') + "-pvgrid-on-extra-search-changed", _this.setExtraSearch);
        };
        _this.dataSource = {
            rowCount: undefined,
        ...
        _this.render = function () {
            ...
            if (!_this.state.hideMenu) {
                menu = (React.createElement(Menu, { noOverlay: true, style: { position: 'absolute', right: '10px' }, pageWrapId: 'outer-wrap', right: true, outerContainerId: 'outer-wrap' },
                    React.createElement(PVGridColSelector
                    // glEventHub={this.props.glEventHub}
                    , { 
                        // glEventHub={this.props.glEventHub}
                        style: { height: '100%', width: '100%' }, namespace: "" + _this.namespace + (_this.subNamespace ? _this.subNamespace : ''), colSettings: _this.state.columnDefs, dataType: _this.dataType })));
            }
            else {
                menu = React.createElement("div", null);
            }
            return (React.createElement("div", { style: { width: '100%', height: 'calc(100% - 20px)' } },
                menu,
                React.createElement("div", { style: { width: '100%', height: '100%' }, className: 'ag-theme-balham-dark', id: 'outer-wrap' },
                    React.createElement(AgGridReact, { columnDefs: _this.state.columnDefs, 
                        // autoGroupColumnDef={this.state.autoGroupColumnDef}
                        defaultColDef: _this.state.defaultColDef, suppressRowClickSelection: true, groupSelectsChildren: true, debug: true, rowSelection: _this.state.rowSelection, 
                        // rowGroupPanelShow={this.state.rowGroupPanelShow}
                        // pivotPanelShow={this.state.pivotPanelShow}
                        enableRangeSelection: false, 
                        // pagination={true}
                        // paginationPageSize={this.state.paginationPageSize}
                        paginationNumberFormatter: _this.state.paginationNumberFormatter, localeTextFunc: _this.state.localeTextFunc, onGridReady: _this.onGridReady, rowData: _this.state.rowData, datasource: _this.dataSource, onRowClicked: _this.onClick, 
                        // components={this.state.components}
                        // rowBuffer={this.state.rowBuffer}
                        rowDeselection: true, rowModelType: _this.state.rowModelType, 
                        // cacheOverflowSize={this.state.cacheOverflowSize}
                        // maxConcurrentDatasourceRequests={this.state.maxConcurrentDatasourceRequests}
                        // infiniteInitialRowCount={this.state.infiniteInitialRowCount}
                        // maxBlocksInCache={this.state.maxBlocksInCache}
                        // paginationPageSize={100}
                        cacheOverflowSize: 2, maxConcurrentDatasourceRequests: 2, infiniteInitialRowCount: 1, maxBlocksInCache: 2, pagination: true, paginationAutoPageSize: true, getRowNodeId: function (item) {
                            return item.id;
                        } }))));
            /*       return (
        ...
        _this.state = __assign(__assign({ hideMenu: true }, _this.props), { totalRecords: 0, columnDefs: _this.getColSettings(props), defaultColDef: {
                editable: false,
            ...
            rowData: _this.data, paginationNumberFormatter: function (params) {
    ...
    PVGrid.prototype.getColSettings = function (props) {
        var colSettingsStr = PontusComponent.getItem("" + this.namespace + (this.subNamespace ? this.subNamespace : '') + ".PVGrid.colSettings");
        var colSettings;
        if (colSettingsStr) {
    ...
    PVGrid.prototype.getDataType = function (props) {
        var dataType = PontusComponent.getItem("" + this.namespace + (this.subNamespace ? this.subNamespace : '') + ".PVGrid.dataType");
